...
(function ($) {
    ...
    function createBox($fileInput, file, webUploader) {
        ...
        webUploader.makeThumb(file, function (error, dataSrc) {
            if (!error) {
                imgChaneBase64(file.source.source, function (result) {
                    $fileBox.find('.viewThumb').append("<img src='" + dataSrc + "' ><input type='hidden'  value='" + JSON.stringify(result) + "'>");
                });
    ...
    //原file文件格式转换
    function imgChaneBase64(file, fun) {
        try {
            var file = file;
