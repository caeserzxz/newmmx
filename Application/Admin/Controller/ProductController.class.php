...
class ProductController extends PublicController{
	...
	public function add(){	
		...
		try{	
			...
			$array=array(
				...
				'shop_type'=>intval($_POST['shop_type']),//所属商城
				// 'maid_one'=>intval($_POST['maid_one']),//白银分成
				// 'maid_two'=>intval($_POST['maid_two']),//黄金分成
				// 'maid_three'=>intval($_POST['maid_three']),//钻石分成
				'shiyong'=>rand(1000,1500),
                'level_id'=>$_POST['level_id'],
                'price_old'=>$_POST['price_old']
			);
			  
			// if($_POST['maid_one']+$_POST['maid_two']+$_POST['maid_three']>=100){
			// 	$this->error('分成参数设置错误');
			// }
			// if(!is_numeric($_POST['maid_one'])||!is_numeric($_POST['maid_two'])||!is_numeric($_POST['maid_three'])){
			// 	$this->error('分成参数设置错误');
			// }
	...
	*/
	...
	public function comment(){
		...
		$list = M('product_dp')->alias('a')->join('LEFT JOIN lr_user b on a.uid=b.id')->field('a.*,b.uname,b.photo,c.name')->order('addtime desc')->join('LEFT JOIN lr_product c on c.id = a.pid')->limit($limit,rows)->select();
		foreach ($list as $k => $v) {
			$list[$k]['ratio'] = $v['num']*10;
		}
		$_REQUEST['page'] = $page;
		$this->assign('page',$page);
		$this->assign('page_index',$page_index);
		$this->assign('productdp',$list);
		$this->display();
	}
	...
	public function comment_detail(){
		if(IS_POST){
			...
			$res = M('product_dp')->where(array('id'=>$data['id']))->save($data);
			if ($res) {
				$this->redirect('Product/comment',array('page'=>intval($data['page'])));
				exit();
			}else{
				$this->error('操作失败.');
				exit();
			...
			$id = I('id');
			$page = I('page');
			$info =M('product_dp')->alias('a')->join('LEFT JOIN lr_user b on a.uid=b.id')->field('a.*,b.uname,b.photo,c.name')->order('addtime desc')->join('LEFT JOIN lr_product c on c.id = a.pid')->where(array('a.id'=>$id))->find();
			$this->assign('page',$page);
