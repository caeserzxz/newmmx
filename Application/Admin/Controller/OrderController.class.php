...
class OrderController extends PublicController{
	...
	*/
	public function _initialize(){
		...
		$order_status = array('10'=>'待付款','20'=>'待发货','30'=>'待收货','40'=>'顾客已自取','32'=>'暂时不发货','40'=>'已收货','50'=>'交易完成');
	...
	*/
	public function sms_up(){
		...
		if ($o_info['kuaidi_name']==$kuaidi_name && $o_info['kuaidi_num']==$kuaidi_num && $o_info['remarks']==$remarks && intval($o_info['status'])==$order_status) {
			$arr = array();
	...
	*/
	public function  withdraw_list(){
		...
		// $where['t1.orderids'] = array('neq','');

		$withdraw = M('withdraw')->alias('t1')->join('left join lr_user t2 on t1.uid = t2.id')->field('t1.*,t2.uname,t2.photo,t2.phone,t2.real_name,t2.bank_name,t2.banks')->where($where)->limit($limit,rows)->order('t1.create_time desc')->select();
	...
	public function carry(){
		$id =I('id');
		$status =I('status');
		$carry = M('withdraw')->where(array('id'=>$id))->find();
		//已审核过的订单不可操作
		if($carry['status']==2||$carry['status']==3){
			$return['status'] = -1;
			$return['msg'] = '已审核的提现,不可重复操作';
			$this->ajaxreturn($return);
		}
		// $orderids = $carry['orderids'];

		// if(empty($orderids)){
		// 	$return['msg'] = "该提现订单有误,不可通过提现或驳回";
		// 	$this->ajaxreturn($return);
		// }
		...
		//修改提现表状态
		$res = M('withdraw')->where(array('id'=>$id))->save($save);
		...
		if($res){
			...
			// 	$saves['status'] = $status;
			// }
			// $saves['svae_time'] = date("Y-m-d H:i:s",time());
			// foreach ($orderids as $k => $v) {
			// 	//修改流水表状态
			// 	M('glide')->where(array('id'=>$v))->save($saves);
			// }

			//修改用户金额
			if($status=='3'){
				$can_withdraw = M('user')->where(array('id'=>$carry['uid']))->getField('can_withdraw');
				$user_save['can_withdraw'] = $can_withdraw+$carry['money'];
				M('user')->where(array('id'=>$carry['uid']))->save($user_save);
			}else if($status=='2'){
                $withdraw_old = M('user')->where(array('id'=>$carry['uid']))->getField('withdraw_old');
                $user_save['withdraw_old'] = $withdraw_old+$carry['money'];
                M('user')->where(array('id'=>$carry['uid']))->save($user_save);
            }
			...
			$return['status'] = -1;
			if($status=='2'){
				$return['msg'] = "确认转账失败,请联系技术人员";
			}else if($status=='3'){
				$return['msg'] = "驳回失败,请联系技术人员";
			}else if($status=='4'){
				$return['msg'] = "审核通过失败,请联系技术人员";
			}
			else if($status=='5'){
				$return['msg'] = "审核不通过失败,请联系技术人员";
			}
			$this->ajaxreturn($return);
		}
		$this->ajaxreturn($carry);
	}
}