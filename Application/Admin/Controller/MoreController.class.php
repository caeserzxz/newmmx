...
class MoreController extends PublicController{
	...
	public function  wechat(){
		if(IS_POST){
			$check = M('wechat_config')->where()->getField('id');
			if(empty($check)){
				$_POST['wx_url'] = "http://jifen.518wk.cn/api.php?id=1";
				$_POST['wx_token'] = generateRandomString2(32);
				$_POST['wx_coding_key'] =generateRandomString(43);
				$_POST['wx_mchkey'] = generateRandomString(32);
			}
			//构建数组
			M('wechat_config')->create();
			//上传产品分类缩略图
			if (!empty($_FILES["file2"]["tmp_name"])) {
				//文件上传
				$info2 = $this->upload_images($_FILES["file2"],array('jpg','png','jpeg'),"logo");
			    if(!is_array($info2)) {// 上传错误提示错误信息
				    ...
				    M('wechat_config')->logo = 'UploadFiles/'.$info2['savepath'].$info2['savename'];
			    }
			}
			M('wechat_config')->uptime=time();

			
			if (intval($check)) {
				$up = M('wechat_config')->where(array('id'=>$check))->save();
			}else{
				M('wechat_config')->id=1;
				$up = M('wechat_config')->add();
			}
			...
			$this->assign('info',M('wechat_config')->find());
			$this->display();
		}
	}
}