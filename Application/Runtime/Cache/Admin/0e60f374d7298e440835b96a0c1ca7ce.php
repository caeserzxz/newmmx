...
<div class="page-container">
    ...
    <table class="table table-border table-bordered table-bg">
        ...
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$adv): $mod = ($i % 2 );++$i;?><tr class="text-c">
                <td><?php echo ($adv["id"]); ?></td>
                <td><?php echo ($adv["title"]); ?></td>
                <td>
                    <?php if($adv["is_nav"] == 1): ?><span class="label succ" style="background:#00FFFF;">是</span>
                    <?php else: ?>
                        <span class="label err">否</span><?php endif; ?>
                </td>
                <td class="obj_1">
                    <a href="<?php echo U('add');?>?adv_id=<?php echo ($adv["id"]); ?>" >修改</a>&nbsp;|
                    <a onclick="del_id_url(<?php echo ($adv["id"]); ?>,<?php echo ($page); ?>)">删除</a>&nbsp;|
                    <?php if($adv["is_nav"] == 1): ?><a  onclick=set_is_nav(2,<?php echo ($adv["id"]); ?>,<?php echo ($page); ?>)>隐藏</a>
                    <?php else: ?>
                        <a  onclick=set_is_nav(1,<?php echo ($adv["id"]); ?>,<?php echo ($page); ?>)>显示</a><?php endif; ?>
                </td>
...
<script>
    ...
    function del_id_url(id,page){
        if(confirm("确认删除吗？"))
        {
            location='<?php echo U("Guanggao/notice_del");?>?id='+id+"&page="+page;
        }
    }
    
    function set_is_nav(type,id,page){
        if(type==1){
            var titile = "您确定要显示吗?";
        }else{
            var titile = "您确定要隐藏吗?";
        ...
        if (confirm(titile)) {
            location.href="<?php echo U('set_is_nav');?>?id="+id+'&type='+type+'&page='+page;
        };
    }
</script>
